  ...
  return (
    <div className="max-w-5xl mx-auto mt-8 p-4">
        ...
        <div className="flex-1 bg-transparent rounded-xl p-8 border border-white/20">
          ...
          <div className="flex items-center justify-center min-h-[320px]">
            <div className="w-80 h-80 flex flex-col items-center justify-center border-2 border-blue-500 rounded-xl">
              <span className="text-white text-2xl font-bold mb-2">Market Cap: $250k</span>
            </div>
          </div>
        </div>
        {/* Widget URL */}
      ...
      <div className="bg-transparent rounded-2xl border border-white/20 p-8 mt-8">
        <h1 className="text-3xl font-bold mb-8 text-center text-white">Market Cap Widget Configuration</h1>
        <ol className="space-y-8">
          ...
          {/* 2. Set Market Cap Goals */}
          <li>
            <div className="font-bold text-lg mb-2 text-white">2. Set Market Cap Goals (USD)</div>
            <p className="text-sm text-gray-400 mb-2">Set up to three market capitalization goals and optional custom text for each. Higher tier goals must be greater than lower ones.</p>
            {goals.map((g, i) => (
              <div className="flex gap-4 mb-2" key={i}>
                <input
                  type="text"
                  className="flex-1 bg-black border border-white/20 rounded-lg px-4 py-3 text-white"
                  placeholder={`Market cap goal ${i + 1}`}
                  value={g.goal}
                  onChange={e => {
          ...
          <li>
            <div className="font-bold text-lg mb-2 text-white">3. Choose Color & Style</div>
