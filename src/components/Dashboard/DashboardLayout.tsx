...
import { Play, DollarSign, MessageSquare, Flame, Target } from 'lucide-react';
...
const DashboardLayout = ({ children }: { children: React.ReactNode }) => {
  ...
  <div className="min-h-screen h-screen w-screen flex bg-gradient-to-br from-[#0a2540] via-[#1e3a5c] to-[#23272f] text-white overflow-hidden" style={{background: 'radial-gradient(ellipse at 20% 40%, #a21caf33 0%, transparent 60%), radial-gradient(ellipse at 80% 80%, #a21caf22 0%, transparent 70%), linear-gradient(120deg, #0a2540 0%, #1e3a5c 60%, #23272f 100%)'}}>
      {/* Fixed Sidebar */}
  <aside className="fixed left-0 top-0 h-full w-56 min-w-48 flex flex-col bg-transparent border-r border-blue-500 px-2 py-6 z-30">
        <div className="mb-8 flex items-center gap-2 px-2">
          <img src="https://streamdotfun.com/images/logo_wide.png" alt="logo" className="h-8" />
        </div>
  ...
  <main className="flex-1 h-full bg-transparent ml-56 overflow-y-auto">
        {children}
      </main>
    </div>
  );
